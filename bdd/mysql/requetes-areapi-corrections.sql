...
INNER JOIN Club b ON (b.idClub = Rencontre.idClubB);
...
INNER JOIN Club ClubB ON (ClubB.idClub = Rencontre.idClubB) 
...
SELECT Score.debut, Score.numeroSet, joueurA.numeroLicence, joueurA.nom, joueurA.prenom, ClubA.nomClub, Score.pointsJoueurClubA, Score.pointsJoueurClubB, joueurB.numeroLicence, joueurB.nom, joueurB.prenom, ClubB.nomClub FROM Partie
INNER JOIN Joueur joueurA ON (joueurA.numeroLicence = Partie.idJoueurA)
INNER JOIN Joueur joueurB ON (joueurB.numeroLicence = Partie.idJoueurB)
INNER JOIN Rencontre ON Partie.idRencontre = Rencontre.idRencontre
INNER JOIN Club ClubA ON (ClubA.idClub = Rencontre.idClubA)
...
INSERT INTO Score(idPartie,numeroSet, pointsJoueurClubA, pointsJoueurClubB, debut) VALUES (1,3,0,0,DATETIME('now'));

---- Finir un set et enregistrer le score

UPDATE Score SET fin=DATETIME('now'), pointsJoueurClubA='11', pointsJoueurClubB='9' WHERE idPartie='1' AND numeroSet='3';
