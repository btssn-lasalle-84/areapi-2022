...
CREATE TABLE IF NOT EXISTS Rencontre(idRencontre INTEGER PRIMARY KEY AUTOINCREMENT, idClubA INTEGER NOT NULL, idClubW INTEGER NOT NULL, nbPartiesGagnantes INTEGER DEFAULT 0, estFinie INTEGER DEFAULT 0, horodatage DATETIME NOT NULL, CONSTRAINT fk_idClub_A FOREIGN KEY (idClubA) REFERENCES Club(idClub), CONSTRAINT fk_idClub_B FOREIGN KEY (idClubW) REFERENCES Club(idClub));
...
CREATE TABLE IF NOT EXISTS Score(idPartie INTEGER NOT NULL, numeroSet INTEGER NOT NULL, pointsJoueurClubA INTEGER DEFAULT 0, pointsJoueurClubW INTEGER DEFAULT 0, debut DATETIME NOT NULL, fin DATETIME, CONSTRAINT pk_score PRIMARY KEY (idPartie,numeroSet), CONSTRAINT fk_idPartie_1 FOREIGN KEY (idPartie) REFERENCES Partie(idPartie));
